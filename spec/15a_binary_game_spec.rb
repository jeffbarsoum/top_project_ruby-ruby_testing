...
describe BinaryGame do
  ...
  describe '#verify_input' do
    ...
    let(:min) { game_verify_input.instance_variable_get(:@minimum) }
    let(:max) { game_verify_input.instance_variable_get(:@maximum) }
    ...
    context 'when given a valid input as argument' do
      ...
      it 'returns valid input' do
        expect(game_verify_input.verify_input(min, max, valid_input)).to be valid_input.to_i
      end
    end
    ...
    context 'when given invalid input as argument' do
      let(:invalid_input) { '(' }
      it 'returns nil' do
        expect(game_verify_input.verify_input(min, max, invalid_input)).to be nil
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    before do
      ...
      allow(game_turn_order).to receive(:display_guess).with(search)
    ...
    it 'increases guess_count by one' do
      expect { game_turn_order.display_turn_order(search) }.to change { game_turn_order.instance_variable_get(:@guess_count) }.by 1
      game_turn_order.display_turn_order(search)
    end
