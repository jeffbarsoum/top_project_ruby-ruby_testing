...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the random number is 4 and the previous step is 11' do
      it 'returns 7' do
        test = described_class.new(4)
        previous_step = 11
        result = test.subtract_random_number(previous_step)
  ...
  describe '#play' do
    context 'when the random number is 10' do
      ...
      it 'will return 7' do
        test = described_class.new(10)
        result = test.play
        expect(result).to eq 7
      end
    end

    context 'when the random number is 125' do
      # remove the 'x' before running this test
      it 'will return 7' do
        test = described_class.new(125)
        result = test.play
        expect(result).to eq 7
      end
    end
